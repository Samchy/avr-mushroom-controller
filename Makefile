...
SRC +=  lph7366.c timer1.c rc5.c cBuffer.c backlight.c Main.c DHT22.c
# USART.c
...
LDFLAGS = -Wl,--defsym=__heap_start=0x002000,--defsym=__heap_end=0x0020ff,-Map=$(PROJECT).map,--cref
...
all: begin gccversion sizebefore $(PROJECT).elf $(PROJECT).hex $(PROJECT).eep \
	$(PROJECT).lss $(PROJECT).sym sizeafter finished end
...
sizeafter:
	@if [ -f $(PROJECT).elf ]; then echo; echo $(MSG_SIZE_AFTER); $(ELFSIZE); echo; fi
